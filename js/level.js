...
export const levelState = {
    ...
    mapBoundingBox: new THREE.Box3(), orbs: [],
    bgMesh: null, parametricMesh: null, levelMesh: null,
...
export function loadLevel(scene, loadingManager, levelFile) {
    ...
    loader.load(levelFile, (gltf) => {
        ...
        // Crear Orbes
        for(let i=0; i<3; i++) {
            const mesh = new THREE.Mesh(new THREE.SphereGeometry(0.3, 16, 16), new THREE.MeshBasicMaterial({ color: 0x00ffff }));
            const light = new THREE.PointLight(0x00ffff, 3, 5);
            mesh.add(light);
            scene.add(mesh);
            levelState.orbs.push(new OrbLogic(i, mesh, light));
            mesh.position.set(0, -9999, 0); 
        }

        // Generar hierba
        setTimeout(() => generateInstancedGrass(scene), 500);
    });
}
...
export function unloadCurrentLevel(scene) {
    ...
    levelState.orbs.forEach(orb => { scene.remove(orb.mesh); orb.mesh.geometry.dispose(); orb.mesh.material.dispose(); });
    levelState.orbs = [];
    if (levelState.levelMesh) { levelState.levelMesh.traverse((c) => { if (c.isMesh) { c.geometry.dispose(); if(c.material.map) c.material.map.dispose(); c.material.dispose(); } }); scene.remove(levelState.levelMesh); levelState.levelMesh = null; }
...
// --- SHADER DE VIENTO Y GENERACIÓN DE HIERBA (SOLUCIÓN LEGIBLE) ---
...
function modifyMaterialForWind(material) {
    ...
    newMat.onBeforeCompile = (shader) => {
        ...
        // Inyectar el uniform de tiempo
        shader.vertexShader = `
            uniform float time;
            ${shader.vertexShader}
        `;
        // Inyectar la lógica de movimiento
